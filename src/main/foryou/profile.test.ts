...
describe('the watchlist', () => {
  ...
  it('leaves out entries kept only for their ticks — they seed "More like your watchlist" otherwise', () => {
    const profile = buildProfile(store({
      watchlist: [saved(1, [18], 100), { ...saved(2, [18], 300), listed: false }],
    }), NOW)
    expect(profile.watchlist.map((w) => w.tmdbId)).toEqual([1])
  })
})
